...
const getjobs = (html: string): Job[] => { // takes in html string and returns Cheerio interface with cheerio elements
...
axios.get(url).then( response => {
    ...
    const jobs: Job[] = getjobs(html); // API provides interface to interact with basically array of list items
    console.log(jobs);
});
